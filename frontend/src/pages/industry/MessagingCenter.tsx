...
export default function MessagingCenter() {
  ...
  const fetchMessages = () => {
    ...
    axios.get("http://127.0.0.1:8000/industry/dashboard/communication/inbox", {
      ...
      .then(res => {
        ...
        msgs.forEach((m: any) => {
           ...
           if (!uniquePartners.has(partnerId)) {
               uniquePartners.set(partnerId, partnerName || `User #${partnerId}`);
           }
        });

        if (studentIdParam) {
           const sId = parseInt(studentIdParam);
           if (!uniquePartners.has(sId)) {
               uniquePartners.set(sId, studentNameParam || `Candidate #${sId}`);
           }
           setActivePartnerId(sId);
  ...
  const handleSendMessage = async (e: FormEvent) => {
    ...
    try {
      const token = localStorage.getItem("token");
      await axios.post("http://127.0.0.1:8000/industry/dashboard/communication/send", {
  ...
  return (
    <DashboardLayout role="industry">
      <div className="max-w-7xl mx-auto h-[calc(100vh-8rem)] flex flex-col">
        <h1 className="text-3xl text-gray-900 mb-8 flex-none">Messaging Center</h1>
